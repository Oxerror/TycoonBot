...
class TycoonSession:
    ...
    def process_frame(self, frame):
        ...
        Returns:
            ...
            detections (for drawing); regions is (hand, field) crops.
        """
        messages = []
        ...
        if self.game_state is None or round_start:
            ...
            self._start_tracking(bar_counts, trick, cards, counters['player'])
            if starting:
            ...
            if not mismatches:
                ...
                # Prefer adopting a bar reading that is stable across
                # two frames — resyncing from a mid-animation misread
                # would poison the state and re-alarm immediately. But
                # never stay diverged forever: after several divergent
                # frames resync regardless.
                if self.diverged_frames >= 2 and (bar_counts == self.previous_bar
            ...
            if None not in opponent_counts:
                ...
                if opponents_total != self.game_state.total_unseen():
                    messages.append(
                        f"Cards-left cross-check: bubbles say {opponents_total}, "
                        f"tracking says {self.game_state.total_unseen()}")
